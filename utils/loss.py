...
class ComputeLoss:
    ...
    #     people_class = 2  
    #     if nt:
    #         ignore_mask = targets[:, 1] == people_class
    #         if ignore_mask.any():
    #             targets[ignore_mask, 1] = -1  
    #     tcls, tbox, indices, anch = [], [], [], []
    ...
    #             j, k = ((gxy % 1 < g) & (gxy > 1)).T
    #             l, m = ((gxi % 1 < g) & (gxi > 1)).T
    #             jk_lm = torch.stack((torch.ones_like(j), j, k, l, m))
    #             t = t.repeat((5, 1, 1))[jk_lm]
    #             offsets = (torch.zeros_like(gxy)[None] + off[:, None])[jk_lm]
    #         else:
    #             t = targets[0]
    #             offsets = 0
    ...
    #         # Define
    #         bc, gxy, gwh, a = t.chunk(4, 1)           # (image, class), grid xy, grid wh, anchors
    #         a, (b, c) = a.long().view(-1), bc.long().T  # anchor, image, class
    #         gij = (gxy - offsets).long()
    #         gi, gj = gij.T  
